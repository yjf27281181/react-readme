...
export function* postQuestion(questionData) {
  ...
  try {
    ...
    yield put({
      ...
      msgContent: "something wrong in server",
      msgType: 0
    });
  } finally {
...
export function* getQuestions(data) {
  ...
  try {
    return yield call(
      get,
      `/question/get?pdfName=${data.pdfName}&pageNum=${data.pageNum}`
    );
  } catch (error) {
    yield put({
      type: IndexActionTypes.SET_MESSAGE,
...
export function* getQuestionsFlow() {
  while (true) {
    ...
    if (response && response.code === 0) {
      yield put({
        type: IndexActionTypes.SET_MESSAGE,
        msgContent: "get questions success",
        msgType: 1
      });
      yield put({
        type: frontActionTypes.RESPONSE_QUESTIONS,
...
export function* getRecentQuestionsFlow() {
  while (true) {
    ...
    if (response && response.code === 0) {
      yield put({
        type: IndexActionTypes.SET_MESSAGE,
        msgContent: "get recent questions success",
        msgType: 1
      });
      yield put({
        type: frontActionTypes.RESPONSE_RECENT_QUESTIONS,
        recentQuestions: response.recentQuestions
      });
    }
  }
}
...
export function* getPDfNames(request) {
  yield put({ type: IndexActionTypes.FETCH_START });
  const response = yield call(get, `/pdf/get/${request.courseName}`);
  if (response && response.code === 0) {
    yield put({
      type: IndexActionTypes.SET_MESSAGE,
      msgContent: "get PDF name success!",
      msgType: 1
    });
    
    yield put({
      type: frontActionTypes.RESPONSE_PDF_NAMES,
      pdf: { courseName: response.courseName, pdfNames: response.pdfNames }
    });
  }
}
...
export function* getPDfNamesFlow() {
  while (true) {
    var request = yield take(frontActionTypes.GET_PDF_NAMES);
    yield fork(getPDfNames, request);
  }
}